...
SECRET_KEY=os.getenv("SECRET_KEY","niftysignals_secret_2024")
ALGORITHM="HS256"; TOKEN_HOURS=12
IST=pytz.timezone("Asia/Kolkata")
MARKET_OPEN=(9,15); MARKET_CLOSE=(15,30)
DB_PATH="signals.db"; PRICE_HISTORY_LEN=30

USERS={
    "admin@nifty.com":"Admin@123","trader1@nifty.com":"Trade@001",
    "trader2@nifty.com":"Trade@002","trader3@nifty.com":"Trade@003",
    "trader4@nifty.com":"Trade@004","trader5@nifty.com":"Trade@005",
    "trader6@nifty.com":"Trade@006","trader7@nifty.com":"Trade@007",
    "trader8@nifty.com":"Trade@008","trader9@nifty.com":"Trade@009",
...
INDICES_LIST=[
    ...
    {"symbol":"^CNXIT","name":"NIFTY IT","yf":"^CNXIT"},
    {"symbol":"^CNXPHARMA","name":"NIFTY PHARMA","yf":"^CNXPHARMA"},
    {"symbol":"^CNXAUTO","name":"NIFTY AUTO","yf":"^CNXAUTO"},
]
...
def load_historical_data():
    ...
    for b in range(0,len(yf_syms),50):
        ...
        try:
            raw=yf.download(byf,period="1y",interval="1d",group_by="ticker",
...
def create_token(email):
    return jwt.encode({"sub":email,"exp":datetime.utcnow()+timedelta(hours=TOKEN_HOURS)},SECRET_KEY,algorithm=ALGORITHM)
...
@app.post("/api/login")
def login(body:dict):
    email=body.get("email","").strip().lower(); pw=body.get("password","")
    if USERS.get(email)!=pw: raise HTTPException(status_code=401,detail="Invalid credentials")
...
@app.get("/api/history")
def get_history_ep(email:str=Depends(verify_token)): return {"history":get_history(500)}

@app.get("/api/indices")
def get_indices(email:str=Depends(verify_token)): return {"indices":list(store["indices"].values())}

@app.get("/api/notifications")
def get_notifications(email:str=Depends(verify_token)):
    return {"notifications":list(store["notifications"])}

@app.post("/api/notifications/read")
def mark_read(email:str=Depends(verify_token)):
    for n in store["notifications"]: n["read"]=True
    return {"ok":True}

@app.get("/api/rsi-screener")
def rsi_screener(min_rsi:float=45,max_rsi:float=60,email:str=Depends(verify_token)):
    result=[]
    for sym,entry in store["stocks"].items():
        rsi=entry.get("rsi14")
...
def get_chart_data(symbol:str,period:str,email:str=Depends(verify_token)):
    symbol=symbol.upper()
    ...
    try:
        df=yf.download(f"{symbol}.NS",period=cfg["period"],interval=cfg["interval"],
        ...
        for dt,row in df.iterrows():
            ...
            rows.append({"date":dt.strftime("%Y-%m-%d %H:%M") if cfg["interval"] in ["5m","30m"] else dt.strftime("%Y-%m-%d"),
                         "open":v(row["Open"]),"high":v(row["High"]),"low":v(row["Low"]),
                         "close":v(row["Close"]),"volume":int(row["Volume"]) if not pd.isna(row["Volume"]) else 0,
...
def get_index_history(symbol:str,period:str,email:str=Depends(verify_token)):
    cfg=PERIOD_MAP.get(period,PERIOD_MAP["3M"])
    try:
        ...
        for dt,row in df.iterrows():
            def v(x): return None if pd.isna(x) else round(float(x),2)
            rows.append({"date":dt.strftime("%Y-%m-%d %H:%M") if cfg["interval"] in ["5m","30m"] else dt.strftime("%Y-%m-%d"),
                         "close":v(row["Close"]),"high":v(row["High"]),"low":v(row["Low"]),"open":v(row["Open"])})
        return {"symbol":symbol,"period":period,"data":rows}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500,detail=str(e))

@app.get("/api/stock/{symbol}")
def get_stock(symbol:str,email:str=Depends(verify_token)):
    symbol=symbol.upper()
